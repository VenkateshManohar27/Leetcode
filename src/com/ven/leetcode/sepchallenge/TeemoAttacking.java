...
public class TeemoAttacking {
	public int findPoisonedDuration(int[] timeSeries, int duration) {
		...
		int attacking = duration;
		int dur = attacking;
		for (int i = 1; i < timeSeries.length; i++) {
			attacking = ((timeSeries[i] - timeSeries[i - 1]) > duration) ? duration : timeSeries[i] - timeSeries[i - 1];
			dur += attacking;
	...
	public static void main(String[] args) {
		...
		System.out.println(a.findPoisonedDuration(new int[] {1,4}, 2));
		System.out.println(a.findPoisonedDuration(new int[] {1,2}, 2));

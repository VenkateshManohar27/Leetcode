 ...
 * is 9 The power of 14 is 17 The power of 15 is 17 The interval sorted by the
 * power value [12,13,14,15]. For k = 2 answer is the second element which is
 * 13. Notice that 12 and 13 have the same power value and we sorted them in
 * ascending order. Same for 14 and 15. Example 2:
 * 
 * Input: lo = 1, hi = 1, k = 1 Output: 1 Example 3:
 * 
 * Input: lo = 7, hi = 11, k = 4 Output: 7 Explanation: The power array
 * corresponding to the interval [7, 8, 9, 10, 11] is [16, 3, 19, 6, 14]. The
 * interval sorted by power is [8, 10, 11, 7, 9]. The fourth number in the
 * sorted array is 7. Example 4:
 * 
 * Input: lo = 10, hi = 20, k = 5 Output: 13 Example 5:
 * 
	...
	public int getKth(int lo, int hi, int k) {
		...
		for (int i = lo, j = 0; i <= hi; i++, j++) {
			...
			if (memo.containsKey(i) ) {
				arr[j].val = memo.get(i);
			} else {
				arr[j].val = getPower(i);
			}
		}

		Arrays.sort(arr, (a,b) ->{return a.val - b.val;});
		return arr[k-1].index;
	}

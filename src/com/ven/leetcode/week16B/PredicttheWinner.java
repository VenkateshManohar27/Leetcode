package com.ven.leetcode.week16B;
 ...
 * Example 1:
 * 
 * Input: nums = [1,5,2] Output: false Explanation: Initially, player 1 can
 * choose between 1 and 2. If he chooses 2 (or 1), then player 2 can choose from
 * 1 (or 2) and 5. If player 2 chooses 5, then player 1 will be left with 1 (or
 * 2). So, final score of player 1 is 1 + 2 = 3, and player 2 is 5. Hence,
 * player 1 will never be the winner and you need to return false. Example 2:
 * 
 * Input: nums = [1,5,233,7] Output: true Explanation: Player 1 first chooses 1.
 * Then player 2 has to choose between 5 and 7. No matter which number player 2
...
class PredictWinnerRec {
	...
	private int helper(int[] nums, int s, int e, int turn) {
		...
		int a = turn * nums[e] + helper(nums, s, e - 1, -turn);
		int b = turn * nums[s] + helper(nums, s + 1, e, -turn);

		return turn == 1 ? Math.max(a, b) : Math.min(a, b);
	}
}

class PredictWinnerRecMemo {
	public boolean PredictTheWinner(int[] nums) {
		Integer [][] memo = new Integer[nums.length][nums.length];
		return helper(nums, 0, nums.length - 1, memo) >= 0;
	}

	private int helper(int[] nums, int s, int e, Integer[][] memo) {
		if (s == e) {
			return  nums[s];
		}
		...
		int a =  nums[e] - helper(nums, s, e - 1, memo);
		int b =  nums[s] - helper(nums, s + 1, e, memo);

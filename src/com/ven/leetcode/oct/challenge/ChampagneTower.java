 ...
 * Constraints:
 * 
 * 0 <= poured <= 109 0 <= query_glass <= query_row < 100
 * 
...
public class ChampagneTower {
	public double champagneTower(int poured, int query_row, int query_glass) {
		...
		int m = query_row - query_glass + 1;
		int n = query_glass + 1;
		double[][] contain = new double[m][n];
		contain[0][0] = poured;
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				double extra = contain[i][j] - 1;
				if (extra <= 0) {
					continue;
				}
				if (i < m - 1) {
					contain[i + 1][j] += 0.5 * extra;
				}
				if (j < n - 1) {
					contain[i][j + 1] += 0.5 * extra;
				}
				contain[i][j] = 1;
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(c.champagneTower(1, 1, 1));
		System.out.println(c.champagneTower(2, 1, 1));
		System.out.println(c.champagneTower(100000009, 33, 17));
	}
}

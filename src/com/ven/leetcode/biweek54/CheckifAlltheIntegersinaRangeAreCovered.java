 ...
 * Explanation: Every integer between 2 and 5 is covered: - 2 is covered by the
 * first range. - 3 and 4 are covered by the second range. - 5 is covered by the
 * third range. Example 2:
 * 
...
public class CheckifAlltheIntegersinaRangeAreCovered {
	...
	public boolean isCovered(int[][] ranges, int left, int right) {
		...
		int i = left, j = 0;
		while (i <= right && j < ranges.length) {

			while (j < ranges.length) {
				while (i >= ranges[j][0] && i <= ranges[j][1]) {
					if (i == right)
						return true;
					i++;
				}
	...
	public static void main(String[] args) {
		...
		CheckifAlltheIntegersinaRangeAreCovered a = new CheckifAlltheIntegersinaRangeAreCovered();
		System.out.println(a.isCovered(new int[][] { { 1, 2 }, { 3, 4 }, { 5, 6 } }, 2, 5));
		System.out.println(a.isCovered(new int[][] { { 1, 10 }, { 10, 20 } }, 21, 21));

		System.out.println(a.isCovered(new int[][] { { 1, 2 }, { 3, 4 }, { 5, 6 } }, 2, 7));
		System.out.println(a.isCovered(new int[][] { { 1, 2 }, { 3, 4 }, { 5, 6 } }, 1, 6));
	}

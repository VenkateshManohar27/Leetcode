 ...
 * and sets the timeToLive. generate(string tokenId, int currentTime) generates
 * a new token with the given tokenId at the given currentTime in seconds.
 ...
 * at the given currentTime. Note that if a token expires at time t, and another
 * action happens on time t (renew or countUnexpiredTokens), the expiration
 ...
 * "generate", "renew", "renew", "countUnexpiredTokens"] [[5], ["aaa", 1],
 * ["aaa", 2], [6], ["bbb", 7], ["aaa", 8], ["bbb", 10], [15]] Output [null,
 ...
 * tokenId "bbb" at time 7. authenticationManager.renew("aaa", 8); // The token
 * with tokenId "aaa" expired at time 7, and 8 >= 7, so at time 8 the renew
 * request is ignored, and nothing happens. authenticationManager.renew("bbb",
 * 10); // The token with tokenId "bbb" is unexpired at time 10, so the renew
 ...
 * tokenId consists only of lowercase letters. All calls to generate will
 * contain unique values of tokenId. The values of currentTime across all the
 * function calls will be strictly increasing. At most 2000 calls will be made
 * to all functions combined.
 * 

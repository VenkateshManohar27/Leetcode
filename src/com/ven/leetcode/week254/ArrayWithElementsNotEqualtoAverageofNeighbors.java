 ...
 * = 2, and the average of its neighbors is (1+4) / 2 = 2.5. When i=2, nums[i] =
 * 4, and the average of its neighbors is (2+5) / 2 = 3.5. When i=3, nums[i] =
 * 5, and the average of its neighbors is (4+3) / 2 = 3.5. Example 2:
 * 
 * Input: nums = [6,2,0,9,7] Output: [9,7,6,2,0] Explanation: When i=1, nums[i]
 * = 7, and the average of its neighbors is (9+6) / 2 = 7.5. When i=2, nums[i] =
 * 6, and the average of its neighbors is (7+2) / 2 = 4.5. When i=3, nums[i] =
 * 2, and the average of its neighbors is (6+0) / 2 = 3.
 * 
 * Constraints:
 * 
 * 3 <= nums.length <= 105 0 <= nums[i] <= 105
 * 
...
public class ArrayWithElementsNotEqualtoAverageofNeighbors {
	public int[] rearrangeArray(int[] nums) {
		...
		while (start <= end) {
			res[index] = (index % 2 == 0) ? nums[start++] : nums[end--];
			index++;
		}
		return res;
	}

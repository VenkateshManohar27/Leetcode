 ...
 * Output [null,500.0,4000.0,800.0,4000.0,4000.0,7350.0,2500.0] Explanation
 * Cashier cashier = new
 * Cashier(3,50,[1,2,3,4,5,6,7],[100,200,300,400,300,200,100]);
 * cashier.getBill([1,2],[1,2]); // return 500.0, bill = 1 * 100 + 2 * 200 =
 * 500. cashier.getBill([3,7],[10,10]); // return 4000.0
 * cashier.getBill([1,2,3,4,5,6,7],[1,1,1,1,1,1,1]); // return 800.0, The bill
 * was 1600.0 but as this is the third customer, he has a discount of 50% which
 * means his bill is only 1600 - 1600 * (50 / 100) = 800.
 * cashier.getBill([4],[10]); // return 4000.0 cashier.getBill([7,3],[10,10]);
 * // return 4000.0 cashier.getBill([7,5,3,1,6,4,2],[10,10,10,9,9,9,7]); //
 * return 7350.0, Bill was 14700.0 but as the system counted three more
 * customers, he will have a 50% discount and the bill becomes 7350.0
 * cashier.getBill([2,3,5],[5,3,2]); // return 2500.0
 * 
 * 
 * Constraints:
 * 
 * 1 <= n <= 10^4 0 <= discount <= 100 1 <= products.length <= 200 1 <=
 * products[i] <= 200 There are not repeated elements in the array products.
 * prices.length == products.length 1 <= prices[i] <= 1000 1 <= product.length
 * <= products.length product[i] exists in products. amount.length ==
 * product.length 1 <= amount[i] <= 1000 At most 1000 calls will be made to
 * getBill. Answers within 10^-5 of the actual value will be accepted as
 * correct.
 * 
...
class Cashier {
	...
	public double getBill(int[] product, int[] amount) {
		...
		for (int i = 0; i < product.length; i++) {
			bill += this.pp[product[i]] * amount[i];
		...
		System.out.println("Cashier.getBill()"+ bill);
		return bill;
	}
}
...
public class ApplyDiscountEverynOrders {
	public static void main(String[] args) {
		...
		cashier.getBill(new int[] { 1, 2, 3, 4, 5, 6, 7 }, new int[] { 1, 1, 1, 1, 1, 1, 1 }); // return 800.0, The bill
																								// was 1600.0 but as
																								// this is the third
																								// customer, he has a
		...
		cashier.getBill(new int[] { 7, 5, 3, 1, 6, 4, 2 }, new int[] { 10, 10, 10, 9, 9, 9, 7 }); // return 7350.0, Bill
																									// was 14700.0 but
																									// as the system
																									// counted three
																									// more customers,

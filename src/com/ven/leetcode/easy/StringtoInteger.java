...
public class StringtoInteger {
	public int myAtoi(String str) {
		...
		for (int i = 0; i < strs[0].length(); i++) {
			try {
				...
				curValue = Long.parseLong(sb.toString());
		...
		} else if (curValue < Integer.MIN_VALUE) {
	...
	public int myAtoi2(String str) {
		...
		while (i < chars.length && (chars[i] >= '0' && chars[i] <= '9')) {
			//append new char which is falling between 0 - 9 to the curValue.
			curValue = (curValue * 10 + (int) chars[i] - '0');
			if (sign * curValue > Integer.MAX_VALUE) {
				return Integer.MAX_VALUE;
			} else if (sign * curValue < Integer.MIN_VALUE) {
				return Integer.MIN_VALUE;
			}
			i++;
		}

		return (int) (sign * curValue);

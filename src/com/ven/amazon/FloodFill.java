...
public class FloodFill {
	public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
		...
		floodFill(image, sr, sc, newColor, startingPixel);
		return image;
	}

	private void floodFill(int[][] image, int sr, int sc, int newColor, int startingPixel) {
		if (sr < 0 || sc < 0 || sr >= image.length || sc >= image[sr].length || image[sr][sc] != startingPixel) {
			return;
		}
		image[sr][sc] = newColor;
		floodFill(image, sr + 1, sc, newColor, startingPixel);
		floodFill(image, sr - 1, sc, newColor, startingPixel);
		floodFill(image, sr, sc + 1, newColor, startingPixel);
		floodFill(image, sr, sc - 1, newColor, startingPixel);
	}

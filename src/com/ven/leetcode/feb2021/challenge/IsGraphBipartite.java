 ...
 * 
 * 
 * Input: graph = [[1,2,3],[0,2],[0,1,3],[0,2]] Output: false Explanation: We
 * cannot find a way to divide the set of nodes into two independent subsets.
 * 
 * 
 * Constraints:
 * 
 * 1 <= graph.length <= 100 0 <= graph[i].length < 100 0 <= graph[i][j] <=
 * graph.length - 1 graph[i][j] != i All the values of graph[i] are unique. The
 * graph is guaranteed to be undirected.
 * 
...
public class IsGraphBipartite {
	...
	private boolean dfs(int u, int color, int[] seen, int[][] g) {
		if (seen[u] != 0) {
			return seen[u] == color;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(g.isBipartite(new int[][] { { 1, 3 }, { 0, 2 }, { 1, 3 }, { 0, 2 } }));
		System.out.println(g.isBipartite(new int[][] { { 1, 2, 3 }, { 0, 2 }, { 0, 1, 3 }, { 0, 2 } }));
	}

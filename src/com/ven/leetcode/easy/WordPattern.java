...
public class WordPattern {
	public boolean wordPattern(String pattern, String str) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if(strToChar.containsKey(st) || charToString.containsKey(c)) {
				if(strToChar.get(st)!= null && !(strToChar.get(st) == c)) return false;
				if(charToString.get(c) != null && !(charToString.get(c).equals(st))) return false;
			} else {
				strToChar.put(st, c);
				charToString.put(c, st);
			}
		}
		return true;
	}

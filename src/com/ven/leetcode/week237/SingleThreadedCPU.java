 ...
 * finishes task 1 and becomes idle. Example 2:
 * 
 * Input: tasks = [[7,10],[7,12],[7,5],[7,4],[7,2]] Output: [4,3,2,0,1]
 * Explanation: The events go as follows: - At time = 7, all the tasks become
 * available. Available tasks = {0,1,2,3,4}. - Also at time = 7, the idle CPU
 * starts processing task 4. Available tasks = {0,1,2,3}. - At time = 9, the CPU
 * finishes task 4 and starts processing task 3. Available tasks = {0,1,2}. - At
 * time = 13, the CPU finishes task 3 and starts processing task 2. Available
 * tasks = {0,1}. - At time = 18, the CPU finishes task 2 and starts processing
 * task 0. Available tasks = {1}. - At time = 28, the CPU finishes task 0 and
 * starts processing task 1. Available tasks = {}. - At time = 40, the CPU
...
class Task {
	...
	public Task(int index, int start, int processingTime) {
		super();
		this.index = index;
		this.enqueueTime = start;
		this.processingTime = processingTime;
	}

	@Override
	public String toString() {
		return "Task [index=" + index + ", start=" + enqueueTime + ", processingTime=" + processingTime + "]";
	}
...
public class SingleThreadedCPU {
	...
	public int[] getOrder(int[][] tasks) {
		...
		PriorityQueue<Task> taskq = new PriorityQueue<>((a, b) -> a.enqueueTime - b.enqueueTime);
		...
		int i = 0;
		int current = taskq.peek().enqueueTime;
		while (!taskq.isEmpty() || !pq.isEmpty()) {
			while (!taskq.isEmpty() && taskq.peek().enqueueTime <= current) {
				pq.add(taskq.poll());
			}
			...
			if (!pq.isEmpty()) {
				Task t = pq.poll();
				res[i++] = t.index;
				current += t.processingTime;
			} else {
				current = taskq.peek().enqueueTime;
			}

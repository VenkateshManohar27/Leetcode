 ...
 * Constraints:
 * 
 * 1 <= Node.val <= 100 Node.val is unique for each node. Number of Nodes will
 * not exceed 100. There is no repeated edges and no self-loops in the graph.
...
public class CloneGraph {
	public Node cloneGraph(Node node) {
        ...
        while(!q.isEmpty()){
            Node n = q.poll();
            Node newNode = createOrGet(n, visited);
            for(Node neighbor : n.neighbors){
                if(!visited.containsKey(neighbor.val)){
                q.add(neighbor);
                }
                Node clone = createOrGet(neighbor, visited);
    ...
    private Node createOrGet(Node n,  HashMap<Integer, Node> v){
        Node node = null;

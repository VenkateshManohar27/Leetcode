...
public class DistributeCandiestoPeople {
	public int[] distributeCandies(int candies, int num_people) {
		...
		int candiesPerIndex = 0;
		int index = 0;
		while (candies > 0) {
			candiesPerIndex++;
			int reduce = (candies > candiesPerIndex) ? candiesPerIndex : candies;
			candies -= reduce;
			arr[index++] += reduce;
			if (index == arr.length) {
				index %= arr.length;
	...
	public static void main(String[] args) {
		...
		ArrayUtility.print(d.distributeCandies(7, 4));
		ArrayUtility.print(d.distributeCandies(10, 3));
		ArrayUtility.print(d.distributeCandies(5000, 10));
	} 

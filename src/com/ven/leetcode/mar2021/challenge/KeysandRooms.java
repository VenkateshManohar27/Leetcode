 ...
 * 
 * 1 <= rooms.length <= 1000 0 <= rooms[i].length <= 1000 The number of keys in
 * all rooms combined is at most 3000.
 * 
...
public class KeysandRooms {
	public boolean canVisitAllRooms(List<List<Integer>> rooms) {
		...
		while (!st.isEmpty()) {
			int index = st.pop();
			if (!visited[index] && index < rooms.size()) {
				visited[index] = true;
				List<Integer> l = rooms.get(index);
				count++;
				if (count == rooms.size())
					return true;
				for (int i : l) {
